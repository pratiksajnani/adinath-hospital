...
const RateLimiter = {
    ...
    limits: {
        ...
        booking: {
            max: 10,
            windowMs: 60 * 60 * 1000,
            message: 'Too many booking requests. Please wait 1 hour.',
        },
        api: { max: 100, windowMs: 60 * 1000, message: 'Too many requests. Please slow down.' },
        feedback: {
            max: 5,
            windowMs: 60 * 60 * 1000,
            message: 'Too much feedback submitted. Please wait.',
        },
    ...
    record(action, identifier = '') {
        ...
        // Clean old attempts
        record.attempts = record.attempts.filter((timestamp) => now - timestamp < limit.windowMs);
