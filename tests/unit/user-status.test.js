...
describe('updateUserStatusWidget() - Receptionist State', () => {
    beforeEach(() => {
        localStorage.setItem('hms_logged_in', 'true');
        localStorage.setItem('hms_role', 'receptionist');
        localStorage.setItem('hms_user_name', 'Reception');
    });

    test('should show receptionist icon (same as staff)', () => {
        updateUserStatusWidget();
        expect(mockElements['user-status-icon'].textContent).toBe('💁');
    });
...
describe('updateUserStatusWidget() - Nurse State', () => {
    beforeEach(() => {
        localStorage.setItem('hms_logged_in', 'true');
...
describe('doLogout()', () => {
    ...
    test('should clear all auth-related items', () => {
        doLogout();
        ...
        expect(localStorage.getItem('hms_doctor_id')).toBeNull();
        expect(localStorage.getItem('currentPatient')).toBeNull();
    });
});
